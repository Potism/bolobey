-- Fix Betting System - Option B Implementation
-- Bet 100 points → Win 200 betting points + 50 stream points

-- 1. First, let's check what we have
SELECT 'Current system status:' as info;

-- Check if user_bets table has stream_points_bonus column
DO $$
BEGIN
    IF NOT EXISTS (
    ...
    ELSE
        RAISE NOTICE 'stream_points_bonus column already exists';
    END IF;
END $$;

-- 2. Fix the place_bet function to use Option B
...
BEGIN
  ...
  -- Calculate stream points bonus (Option B: 50% of profit)
...
-- 3. Fix the process_bet_payouts function for Option B
...
BEGIN
    ...
    LOOP
        ...
        IF bet_record.bet_on_player_id = winner_id THEN
            -- Option B: Betting points = full winnings, Stream points = 50% of profit
...
-- 4. Create trigger to automatically process payouts
...
-- 5. Update existing bets with stream points bonus
UPDATE user_bets 
...
DO $$
BEGIN
  RAISE NOTICE '✅ Betting system fixed for Option B!';
  RAISE NOTICE '📊 Bet 100 points → Win 200 betting points + 50 stream points';
  RAISE NOTICE '🎯 Stream points = 50%% of bet amount';
  RAISE NOTICE '💰 Betting points = full winnings (bet + profit)';
