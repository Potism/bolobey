...
-- Remove the automatic trigger first (then the function)
DROP TRIGGER IF EXISTS on_tournament_created ON tournaments;
DROP TRIGGER IF EXISTS set_tournament_stream_defaults ON tournaments;
DROP FUNCTION IF EXISTS set_default_stream_settings() CASCADE;
