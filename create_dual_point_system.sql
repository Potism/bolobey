-- Complete Dual-Point System for V3
-- Betting Points (purchasable) + Stream Points (earned)

-- 1. Enhanced User Points Table
...
-- 2. Point Packages Table (for purchasing betting points)
...
-- 3. Point Transactions Table (track all point movements)
CREATE TABLE IF NOT EXISTS point_transactions (
    ...
    reference_id UUID, -- tournament_id, match_id, prize_id, etc.
    reference_type VARCHAR(50), -- 'tournament', 'match', 'prize', etc.
    description TEXT,
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 4. Insert default point packages
INSERT INTO point_packages (name, betting_points, price_eur, bonus_points, is_featured) VALUES
...
GRANT ALL ON user_points TO admin;
GRANT ALL ON point_packages TO admin;
GRANT ALL ON point_transactions TO admin;

-- 9. Create functions for point management

-- Function to handle new user points initialization
CREATE OR REPLACE FUNCTION handle_new_user_points()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO user_points (user_id, betting_points, stream_points)
    VALUES (NEW.id, 100, 0); -- Give 100 free betting points to new users
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to add betting points (purchase)
...
-- Function to spend betting points (place bet)
...
-- Function to add stream points (win bet)
...
-- Function to spend stream points (redeem prize)
...
-- Point transaction history view
...
-- 12. Grant view permissions
GRANT SELECT ON user_points_summary TO authenticated;
GRANT SELECT ON point_transaction_history TO authenticated;
GRANT SELECT ON user_points_summary TO admin;
GRANT SELECT ON point_transaction_history TO admin;

-- 13. Success message
DO $$
BEGIN
    RAISE NOTICE 'Dual-point system setup complete!';
    RAISE NOTICE 'Betting Points (purchasable) + Stream Points (earned) system ready.';
    RAISE NOTICE '6 point packages created with pricing from €5 to €200.';
    RAISE NOTICE 'New users automatically get 100 free betting points.';
END $$; 