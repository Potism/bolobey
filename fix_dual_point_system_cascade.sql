-- Fix Dual-Point System - Drop with CASCADE to handle dependencies
...
DROP FUNCTION IF EXISTS spend_stream_points(UUID, INTEGER, UUID, TEXT, TEXT) CASCADE;
...
DROP VIEW IF EXISTS point_transaction_history CASCADE;
...
-- 4. Insert point packages (only if table is empty)
...
-- 11. Create trigger (this will replace the existing one)
...
BEGIN
    ...
    RAISE NOTICE 'Existing trigger dependencies have been resolved.';
END $$; 