-- Simplified Betting System Fix (No Win Streaks)
-- Run this in Supabase SQL Editor to fix the betting system error

-- 1. Drop the problematic function with CASCADE to remove dependencies
DROP FUNCTION IF EXISTS process_bet_payouts() CASCADE;

-- 2. Create simplified function without win streaks
...
-- 3. Create simplified betting_matches function
...
DO $$
BEGIN
  RAISE NOTICE '✅ Simplified betting system created successfully!';
  RAISE NOTICE '🎯 Win streaks removed for now - system is stable';
  RAISE NOTICE '💰 Betting points and stream points will work correctly';
  RAISE NOTICE '🚀 Live betting should now work without errors';
  RAISE NOTICE '📝 Win streaks can be added back later when needed';
END $$; 