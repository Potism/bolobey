-- Final Fix for Stream Points Constraint
...
-- Step 3: Create a very permissive constraint
...
-- Step 4: Drop the existing function first
DROP FUNCTION IF EXISTS redeem_prize(UUID, UUID);

-- Step 5: Create the new redeem_prize function
...
GRANT EXECUTE ON FUNCTION redeem_prize(UUID, UUID) TO authenticated;

-- Success message
SELECT '✅ Final fix applied! Prize redemption should now work.' as status; 