-- Simple Supabase Service Check - Basic Functionality
...
-- 7. Check for any active locks or issues
SELECT '=== SYSTEM STATUS ===' as section;

-- Check for active locks
SELECT 
  CASE 
    WHEN EXISTS (SELECT 1 FROM pg_locks WHERE locktype = 'relation') 
    THEN '⚠️ Active locks detected'
    ELSE '✅ No active locks'
  END as lock_status;

-- Check database size (if accessible)
DO $$
BEGIN
  RAISE NOTICE 'Database size check: Run this in Supabase Dashboard > Database > Settings';
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Cannot check database size: %', SQLERRM;
END $$;

-- 8. Test manual auth insert (simplified)
