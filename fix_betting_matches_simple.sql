-- Simplified Betting Matches Integration
...
-- 2. Ensure user_bets table has stream_points_bonus column
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 4. Create function to process betting_matches payouts
    ...
    IF NEW.status = 'completed' AND NEW.winner_id IS NOT NULL THEN
        ...
        LOOP
            ...
            IF bet_record.bet_on_player_id = NEW.winner_id THEN
                ...
                stream_points_bonus := FLOOR(bet_record.points_wagered * 0.5);
...
-- 5. Create trigger for betting_matches
...
-- 6. Create a function to place bets on betting_matches
...
-- 8. Process any existing unprocessed bets from betting_matches
DO $$
DECLARE
    bet_record RECORD;
    user_betting_points INTEGER;
